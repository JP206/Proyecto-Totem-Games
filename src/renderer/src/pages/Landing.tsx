...
const Landing: React.FC = () => {
    ...
    try {
      ...
      try {
        ...
        const csvFile = files.find(
          (f) => f.isFile && f.name === `${name}_localizar.csv`,
        );
        const xlsxFile = files.find(
          (f) => f.isFile && f.name === `${name}_localizar.xlsx`,
        );
      ...
      await Promise.all([loadContexts(path), loadGlossaries(path)]);
    } catch (error) {
    ...
    try {
      ...
      setGlossaryFiles(
        files
          .filter(
            (f) =>
              f.isFile && (f.name.endsWith(".csv") || f.name.endsWith(".xlsx")),
          )
          .map((f, i) => ({
            name: f.name,
            path: f.path,
  ...
  const processFileUpload = async (
    type: "context" | "glossary" | "localize",
  ) => {
  ...
  const toggleRegion = (region: string, languages: Language[]) => {
    ...
    const allSelected = regionIds.every((id) =>
      targetLanguages.some((l) => l.id === id),
    );
    ...
    setTargetLanguages((prev) =>
      allSelected
        ...
        : [
            ...prev,
            ...languages.filter((l) => !prev.some((p) => p.id === l.id)),
          ],
    );
  };
      ...
      if (spellCheckBeforeTranslate) {
        ...
        navigate("/translation-preview", {
          state: {
            spellCheckOnly: true,
            fileInfo: {
              filePath: spellResult.filePath,
              csvContent: spellResult.csvContent,
            },
      ...
      <div className="section-header">
        ...
        <div className="section-actions">
          ...
          <button
            ...
            title={show ? "Ocultar lista" : "Mostrar lista"}
          >
        ...
        <div className="landing-content">
          ...
          <div className="config-panel">
            ...
            {renderFileList(
              ...
              setShowGlossaries,
              (i: number) =>
                toggleSelection(glossaryFiles, setGlossaryFiles, i),
              () => toggleAll(glossaryFiles, setGlossaryFiles),
              (i: number) => moveItem(glossaryFiles, setGlossaryFiles, i, "up"),
              (i: number) =>
                moveItem(glossaryFiles, setGlossaryFiles, i, "down"),
            )}
            ...
            <div className="info-note">
              <AlertCircle size={16} />
              <span>
                Los archivos se usarán en el orden de prioridad indicado
              </span>
            </div>
          </div>
          ...
          <div className="work-panel">
            ...
            >
              ...
              <p className="drop-description">
                .txt para contextos • .csv/.xlsx para glosarios o archivo a
                localizar
              </p>
              <input
                id="file-upload"
            ...
            <div className="spellcheck-option">
              <label className="spellcheck-label">
                <input
                  ...
                  onChange={(e) =>
                    setSpellCheckBeforeTranslate(e.target.checked)
                  }
                />
                <span>Revisar ortografía/gramática antes de traducir</span>
              </label>
              <small className="spellcheck-note">
                Revisión ortográfica y gramatical con IA (mismos proveedores que
                la traducción).
              </small>
            </div>
            ...
            <div className="action-section">
              <button
                ...
                disabled={
                  !selectedFile || targetLanguages.length === 0 || translating
                }
              >
                {translating ? (
                  <>
